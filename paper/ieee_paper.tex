...
Section~\ref{sec:future} elaborates on these limitations and presents
five research directions that address them systematically.

% ─────────────────────────────────────────────────────────────
\section{Future Work}\label{sec:future}

The experimental evaluation of Gait-YOLO has exposed several concrete
limitations that define actionable research trajectories.  We outline
five directions, each grounded in specific failure modes observed in
the preceding analysis.

\subsection{Online Threshold Adaptation for Deployment Robustness}

As demonstrated in the gait module deployment (\S\ref{sec:results}),
score distributions on real target data can deviate substantially from
offline calibration assumptions --- real CASIA-B scores cluster near
0.063 whereas simulated thresholds assumed 0.43, producing complete
detection failure until manual recalibration.
Streaming quantile estimators~\cite{greenwald2001} and Bayesian
threshold updating~\cite{adams2007} offer principled mechanisms for
adapting decision boundaries as the system observes deployment data in
real time.  Integrating such online calibration would enable Gait-YOLO
to self-adjust across heterogeneous camera environments, lighting
conditions, and scene contexts without operator intervention ---
directly addressing what our experiments reveal as the single most
critical failure mode in reconstruction-based anomaly detectors.

\subsection{Pose-Skeleton and Optical Flow Based Gait Representation}

The current gait autoencoder operates on binary silhouette sequences
that are inherently sparse (approximately 95\% black pixels in
CASIA-B), causing reconstruction error scores to be bounded by
silhouette darkness rather than motion pattern content.  This
fundamentally limits inter-class separability to $1.7\sigma$ and
caps gait F1 at 0.792.
Replacing silhouette-based reconstruction with pose-skeleton sequences
derived from keypoint detectors such as HRNet~\cite{sun2019hrnet},
or with dense optical flow fields computed between consecutive frames,
would encode motion geometry and velocity directly.  These
representations are invariant to silhouette sparsity and are expected
to increase inter-class separation substantially, enabling the gait
branch to function as a stronger primary detector rather than purely a
corroborative signal.

\subsection{Cross-Modal Attention Feedback Between Branches}

The parallel architecture processes all three modalities independently,
with no inter-branch communication until the final fusion stage.
High-confidence signals in one branch therefore provide no dynamic
influence over the sensitivity of the other branches during inference.
Cross-modal attention mechanisms~\cite{tsai2019multimodal} offer a
principled solution: the confidence output of each branch could
dynamically re-weight the feature extraction or threshold sensitivity
of the remaining branches within the same temporal window.  For
example, a high-confidence weapon detection by YOLO could lower the
anomaly score threshold for VideoMAE and gait within that clip,
enabling the system to respond to corroborating weak evidence that
would otherwise fall below individual branch thresholds.  This
represents a shift from score-level fusion to feature-level
cross-modal interaction, aligning the system more closely with human
...
Gait-YOLO currently operates at the clip level, with each 16-frame
inference window treated independently and no information carried
across successive clips.  This architecture cannot model gradual
behavioral escalation --- a common pattern in real criminal events
where suspicious behavior builds over minutes before a crime
occurs~\cite{ucfcrime}.
Integrating a recurrent temporal memory module, implemented as an
LSTM~\cite{hochreiter1997lstm} or a Transformer decoder operating
over rolling fusion score vectors~\cite{dai2019transformerxl}, would
maintain an evolving suspicion state across extended observation
...
across several minutes indicates genuine threat --- particularly
relevant for covert anomaly classes such as Shoplifting
(R\,=\,0.952) and Robbery (R\,=\,0.800), which exhibit low
per-clip anomaly signal but sustained behavioral deviation.
