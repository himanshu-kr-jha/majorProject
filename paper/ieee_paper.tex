...
\\Delhi, India\\
himanshukrjha004@gmail.com}
\and
\IEEEauthorblockN{Ketan Anand}
\IEEEauthorblockA{\textit{Dept. of  Software Engineering}\\
\textit{Delhi Technological University}
\\Delhi, India\\
ketananand0110@gmail.com}
\and
...
% ─────────────────────────────────────────────────────────────o
\begin{abstract}
Traditional single-stream CCTV setups frequently struggle with high false-alarm rates, often hitting 15 to 25\% depending on the modality. To tackle this, we developed Gait-YOLO, a three-branch anomaly detection framework designed to process multiple video streams in parallel. The system simultaneously runs YOLOv8n to identify weapons, VideoMAE to classify violent behavior, and a CNN-Transformer autoencoder to score gait anomalies. Rather than keeping these outputs isolated, we integrate them using a two-tier fusion layer. This setup relies on a rule-based hierarchical cascade that feeds into a lightweight MLP ($3 \rightarrow 32 \rightarrow 16 \rightarrow 4$), allowing the model to jointly learn the decision boundaries across all three modalities. We evaluated the framework on real-world data, including 324 weapon images of guns and knives, 600 silhouette sequences from CASIA-B, and 190 UCF-Crime videos. The performance is highly competitive: weapon detection reached an mAP@0.5 of 0.724, and the gait stream achieved an F1 score of 0.792 after optimizing the data-calibrated threshold. Action recognition via VideoMAE performed exceptionally well on UCF-Crime, securing an F1 of 0.965 alongside an AUC-ROC of 0.980. Notably, our hierarchical fusion approach achieves perfect recall (1.000). It manages this through a specialized MEDIUM branch that leverages gait corroboration to catch false negatives originally missed by VideoMAE. At the same time, the dedicated YOLO stream maintains a flawless 1.000 precision for weapon-specific events while running at 35 FPS. For optimal overall performance, combining the YOLO and VideoMAE streams yields an F1 score of 0.969 and runs at a steady 15 FPS on standard commodity GPU hardware.
\end{abstract}
...
Automated regional anomaly detection in surveillance videos has progressed from
basic motion thresholding to deep learning pipelines that identify weapons,
listen for violent actions, and search for oddities in crowds. Yet
single-modality systems turn brittle: a pistol detector fires on phones,
an action classifier confuses an indeterminate posture as an attack, and a
gait model flags abnormality at the slightest of variations. Recent studies
point to false-alarm rates of 15-20\% and 12-18\% for action classifiers and
weapon detectors and 18-25\% for gait models when running in isolation~\cite{srilakshmi2025}.
Crucially, we observe that \emph{cross-modal consistency is informative}:
when an alert is cross-validated across several modalities, the system is more
confident of a true alarm, whereas an alert from a single modality with no
corroboration is often a false alarm.
...
Section~\ref{sec:future} outlines five concrete future research
directions motivated by the observed limitations.
...
CASIA-B~\cite{casiab} with 124 subjects and 11 viewpoints is the
benchmark dataset for silhouette based gait analysis.
...
\centering
\tikzset{
  block/.style={rectangle, draw, align=center, rounded corners=3pt, font=\small, fill=blue!15, minimum height=0.7cm, minimum width=2.5cm},
  proc/.style={rectangle, draw, align=center, rounded corners=3pt, font=\small, fill=gray!15, minimum height=0.7cm},
  mlpbox/.style={rectangle, draw, align=center, rounded corners=3pt, font=\footnotesize, fill=yellow!25, minimum height=0.6cm, minimum width=2cm},
  alert/.style={rectangle, draw, align=center, rounded corners=3pt, font=\small\bfseries, minimum width=1.8cm, minimum height=0.6cm},
...
\begin{tikzpicture}[node distance=0.5cm and 0.5cm]
  ...
  \node[proc, minimum width=5cm] (input) {Live CCTV Video Stream};
  \node[proc, below=0.4cm of input, minimum width=5cm] (preproc) {Frame Extraction \& Preprocessing};
  \draw[arr] (input) -- (preproc);

  % 2. Processing Branches (Anchored from the center node for symmetry)
  \node[block, below=0.7cm of preproc] (b2) {VideoMAE\\(every 4th)};
  \node[block, left=0.6cm of b2] (b1) {YOLOv8n\\(every frame)};
  \node[block, right=0.6cm of b2] (b3) {Gait-AE\\(every 4th)};

  % Distribution Arrows
  \draw[arr] (preproc.south) -- ++(0,-0.2) -| (b1.north);
  \draw[arr] (preproc.south) -- (b2.north);
  \draw[arr] (preproc.south) -- ++(0,-0.2) -| (b3.north);

  % 3. Thresholds / Conditions
  \node[mlpbox, below=0.4cm of b1] (c1) {conf $\geq$ 0.60};
  \node[mlpbox, below=0.4cm of b2] (c2) {score $\geq$ 0.965};
  \node[mlpbox, below=0.4cm of b3] (c3) {err $>$ $\tau^*$};
  ...
  % 4. Fusion Layer
  \node[proc, fill=purple!15, below=0.8cm of c2, minimum width=7.2cm] (fusion) {Rule Cascade + MLP Fusion Head};

  % Entering Fusion Layer (Explicit xshifts to avoid clipping rounded corners)
  \draw[arr] (c1.south) -- ++(0,-0.25) -| ([xshift=-2.4cm]fusion.north);
  \draw[arr] (c2.south) -- (fusion.north);
  \draw[arr] (c3.south) -- ++(0,-0.25) -| ([xshift=2.4cm]fusion.north);

  % 5. Alert Outputs (Symmetrically spaced out)
  \node[alert, fill=red!35, below=0.7cm of fusion, xshift=-2.85cm] (cr) {CRITICAL};
  \node[alert, fill=orange!45, below=0.7cm of fusion, xshift=-0.95cm] (hi) {HIGH};
  \node[alert, fill=yellow!55, below=0.7cm of fusion, xshift=0.95cm] (me) {MEDIUM};
  \node[alert, fill=green!35, below=0.7cm of fusion, xshift=2.85cm] (lo) {LOW};

  % Exiting Fusion Layer
  \draw[arr] (fusion.south) -- ++(0,-0.2) -| (cr.north);
  \draw[arr] (fusion.south) -- ++(0,-0.2) -| (hi.north);
  \draw[arr] (fusion.south) -- ++(0,-0.2) -| (me.north);
  \draw[arr] (fusion.south) -- ++(0,-0.2) -| (lo.north);
...
A \emph{temporal persistence filter} needs confidence $\geq0.60$ for five consecutive frames before triggering a CRITICAL alert, ignoring brief false positives from look-alike items (e.g., phones, wallets).

\subsection{Module 2: Action Recognition (VideoMAE)}

The VideoMAE backbone~\cite{tong2022} is a 12-layer ViT-B with 768 hidden dimension and 12 attention heads, pre-trained on Kinetics-400 using masked video autoencoding.
...
Table~\ref{tab:fp} lists FP rates in various configurations
(Fig.~\ref{fig:qualitative} shows example images for each decision class).
Standalone YOLO on weapons has 43.9\% FPR (no persistence filter);
on UCF-Crime surveillance videos it falls to 0\% because weapons
have low probability of occurrence in "normal" videos, proving it as a safe
weapon-specific trigger.
VideoMAE-Only has FPR=16\% on UCF-Crime - the 8 false positives
are all videos with intense spatio-temporal motion (crowds, traffic).
The YOLO+VideoMAE system has FPR=16\% with weapon-event recall
perfect recall of weapon events; the Full Rule system sacrifices
perfect recall (FPR=26\%).
...
\bottomrule
\multicolumn{3}{l}{\footnotesize Full Rule: R=1.000 with higher FPR.}\\
...
Fig.~\ref{fig:qualitative} illustrates three sample fusion decisions on
UCF-Crime test clips (four sampled frames with header)
scores).
\emph{Arson011} (Fig.~\ref{fig:qualitative}a) is a clear case of a true positive:
VideoMAE 0.9998, gait error 0.067, rule and MLP heads both HIGH
HIGH with high confidence.
\emph{Robbery050} (Fig.~\ref{fig:qualitative}b) is the key fusion
recovery case: VideoMAE scores only 0.910 (below $\tau^*$=0.9654, a miss),
but the YOLO branch identifies a weapon with confidence 0.875 and raises the
the alert to CRITICAL - a false negative averted only by multimodal
corroboration.
\emph{Normal\_Videos\_704} (Fig.~\ref{fig:qualitative}c) is the most common
failure mode: a normal corridor scene with people moving causes VideoMAE to
0.987 without any weapon ($c_y$=0.148) or gait anomaly ($s_g$=0.066) to
corroborate; contrastive fine-tuning on such hard-negative normal videos is a
planned improvement.    
...
improvement.}
\caption{Qualitative fusion decisions for three sample UCF-Crime test
videos (4 randomly sampled frames per strip).
\textbf{(a)} True HIGH alert for a clear arson; the the VideoMAE score is
near unity.
\textbf{(b)} Fusion recovery: VideoMAE fails to detect an anomaly (score below
$\tau^*$0.9654), but the YOLO branch spots a weapon and raises to
\textsc{Critical}, showing the importance of corroboration.
\textbf{(c)} Fig.~\ref{fig:pr} shows the precision-recall plot for YOLOv8n weapon
figures causing VideoMAE to exceed the threshold without weapon or human gait corroboration.}
\label{fig:qualitative}
\end{figure*}

\subsection{Comparison with Prior Work}

Table~\ref{tab:compare} compares Gait-YOLO to other systems.
While single-modality precision reports are higher than ours (e.g., Ingle \& Kim report 97.5\% for guns), those systems do not address false-alarm problems.
(e.g., Ingle \& Kim report 97.5\% for guns), these do not tackle
the global false-alarm problem.
Our MLP fusion system is an order of magnitude reduction in FP
single-modality method, with comparable detection accuracy.
...
Fig.~\ref{fig:pr} presents the precision-recall (PR) curves of YOLOv8n
detection.
Knife has higher precision at a given recall, confirming it as
as the trigger for the CRITICAL alert.
...
\begin{axis}[
  ...
  % Add scaled x ticks=false here to remove the 10^-2
  scaled x ticks=false,
  % Force fixed format so it prints as 0.055, 0.060, etc.
  xticklabel style={
...
(mean $\approx$ 0.063).
This is due to silhouette images being $\approx$95\% black pixels,
and ensuring small MSE and SSIM no matter the motion pattern.
The gait module, after recalibration ($\tau^*=0.0642$) has F1=0.792
and can contribute substantially to the fusion via the MEDIUM branch.

\textbf{Limitations.}
VideoMAE has FPR=16\% on normal UCF-Crime videos - crowded scenes (intersections,
with busy normal events (crowds, sport) mis-classed as anomalous.
We plan to have a fine-tuning stage on normal-vs-abnormal
on UCF-Crime should resolve this.
The gait module reaches F1=0.792; fine-tuning on labeled abnormal
data (carrying, disguise) from CASIA-B should produce an F1 greater than 0.85.
The current deployment is on a server-grade GPU; INT8 quantization with
TensorRT for edge devices (NVIDIA Jetson Orin/Xavier) is planned.
Section~\ref{sec:future} discusses these issues and proposes
five directions that address them comprehensively.

% ─────────────────────────────────────────────────────────────
\section{Future Work}\label{sec:future}

The testing of Gait-YOLO has identified a number of practical
areas that provide concrete directions for research.  We outline
five opportunities, each based on distinctive failure modes from
the preceding analysis.

\subsection{Online Threshold Adaptation for Deployment Robustness}

As we show with the gait module deployment (\S\ref{sec:results}),
the real target data score distribution may be significantly different from
offline assumptions (coverage plots for CASIA-B data show)
0.063 while simulation assumed 0.43, resulting in total failure to
total failure until recalibration.
Online quantile updates~\cite{greenwald2001} and Bayesian
threshold updating~\cite{adams2007} provide principled ways of
adjusting the decision boundaries as the system sees data during use
real time.  Incorporation of such online trained decision boundaries would allow Gait-YOLO
to automatically adapt to diverse camera configurations, lighting
lighting, and scene content without user intervention, alleviating what these experiments reveal is the most critical point
breaking the single most important limitation found in our
most common problem when using reconstruction-based detectors.

\subsection{Pose-Skeleton and Optical Flow-Based Gait Representation}

The current gait autoencoder uses binary skeletal silhouettes
that are sparse (95\% black pixels for CASIA-B
CASIA-B), thus limiting the potential scores of reconstruction errors
shell darkness, not shape or motion.  This
gives a fundamental limit on separability of $1.7\sigma$ and
caps gait F1 at 0.792.
Substituting silhouettes for pose-skeleton predictions
based on pose-skeleton estimation networks like HRNet~\cite{sun2019hrnet},
or with dense optical flow fields between frames,
would explicitly represent motion geometry and motion velocity.  These
representations are independent to the sparsity of the silhouette;
to significantly improve the margin between classes and allow the
branch to be a better primary detector, rather than simply
corroborative signal.


\subsection{Cross-Modal Attention Feedback Between Branches}

Parallel processing of the three modalities occurs in isolation,
without communication between branches until the end.
Therefore sensitivity of the branches cannot be affected by
control over the sensitivity of other branches during inference.
Multimodal attention mechanisms~\cite{tsai2019multimodal} provide a
sensible approach: the sensitivity of each branch could
could adaptively weight the weights or the threshold
of the other branches in the same time window.  For
for instance, a high-confidence weapon detection by YOLO could reduce the
the anomaly score threshold of VideoMAE and gait for that clip,
allowing it to react to supportive weak evidence that
would be disregarded when using branch by branch integration.  This
is a shift from score- to feature-level
fusion, bringing the system in line with human
attentional reasoning in threat
assessment~\cite{vaswani2017attention}.

\subsection{Long-Term Temporal Memory for Behavioral Escalation}

Gait-YOLO inference is currently done on a clip-by-clip
inference window, with no temporal memory.
across successive clips.  It cannot capture progressive
escalation - prevalent in real-life crimes
in which anomalous behavior ramps up for several minutes
occurs~\cite{ucfcrime}.
Integrating a recurrent temporal memory with an
LSTM~\cite{hochreiter1997lstm} or a Transformer decoder with
operating over rolling fusion score vectors~\cite{dai2019transformerxl}, would
store a dynamic level of suspicion over time
windows.  This would allow the system to accumulate weak but
consistent anomaly signals over time, flagging situations where no
single clip crosses an alert threshold but the aggregate pattern
across several minutes indicates a genuine threat—particularly
relevant for covert anomaly classes such as shoplifting
(R\,=\,0.952) and robbery (R\,=\,0.800), which exhibit low
per-clip anomaly signals but sustained behavioral deviation.


\subsection{Person Re-Identification Across Camera Networks}

Gait-YOLO operates as a single-camera system with no mechanism for
linking observations of the same individual across multiple camera
feeds.  In practice, CCTV infrastructures consist of overlapping and
non-overlapping camera networks; a flagged individual who exits one
camera's field of view and enters another is treated as an entirely
new subject with all accumulated suspicion context reset.
Person re-identification using appearance embeddings and gait
signatures as a soft biometric~\cite{ye2021deep} --- leveraging the
gait autoencoder's learned latent representations as a
camera-view-invariant identity descriptor --- would enable
network-level threat tracking.  A subject flagged as anomalous on
one camera could be continuously monitored across the entire
surveillance network, substantially extending the system's
operational scope.  Recent advances in view-invariant gait
recognition~\cite{chao2019gaitset} suggest that the 512-dimensional
latent space learned by our CNN--Transformer encoder already captures
discriminative identity features suitable for cross-view matching.
...
The modular architecture supports five concrete future extensions
(Section~\ref{sec:future}), including online threshold adaptation,
pose-skeleton gait representations, cross-modal attention feedback,
long-term temporal memory, and person re-identification across camera
networks --- none of which require changes to the core fusion layer.
...
in \textit{Proc.\ ACM SIGMOD}, pp.~58--66, 2001.
...
in \textit{Proc.\ IEEE/CVF CVPR}, pp.~5693--5703, 2019.
...
in \textit{Proc.\ ACL}, pp.~2978--2988, 2019.

\bibitem{ye2021deep}
M.~Ye \textit{et al.},
``Deep learning for person re-identification: A survey and outlook,''
\textit{IEEE Trans.\ Pattern Anal.\ Mach.\ Intell.}, vol.~44, no.~6,
pp.~2872--2893, 2022.

\bibitem{chao2019gaitset}
H.~Chao \textit{et al.},
``GaitSet: Regarding gait as a set for cross-view gait recognition,''
in \textit{Proc.\ AAAI}, vol.~33, pp.~8126--8133, 2019.
