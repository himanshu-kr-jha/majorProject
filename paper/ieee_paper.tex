...
\title{Gait-YOLO: Real-Time Multimodal Anomaly Detection via\\
       Hierarchical Fusion of Object Detection, Action Recognition,\\
       and Gait Analysis}

\author{
\IEEEauthorblockN{Himanshu Kumar Jha}
\IEEEauthorblockA{\textit{Dept.\ of CSE, [University Name]}\\
[City], [Country]\\
...
silhouette sequences, and 45 UCF-Crime videos — the system achieves
mAP@0.5\,=\,0.724 on weapon detection, Gait\,F1\,=\,0.665 after
data-calibrated threshold optimization, and a fusion F1 of \textbf{0.943}
with precision 0.998.
Compared to a YOLO-only baseline, the false positive rate drops from 43.9\%
to 1.8\%, a \textbf{95.8\% reduction}, while maintaining 18--22\,FPS on
...
\begin{enumerate}
  ...
  \item An MLP fusion head trained on bootstrap samples labeled by the
        rule-based logic, achieving fusion F1\,=\,0.943 and 95.8\% FP
...
\begin{tikzpicture}[node distance=0.52cm and 0.3cm]
  \node[block, fill=gray!15, minimum width=5.5cm] (in)
      {Input: $(c_\text{YOLO},\; p_\text{act},\; s_\text{gait})$};

  % Rule path
  \node[decision, below left=0.65cm and 0.6cm of in] (d1)
      {$c_\text{YOLO}{\geq}0.60$?};
  \node[decision, below=0.58cm of d1] (d2)
      {$p_\text{act}{\geq}0.75$?};
  \node[decision, below=0.58cm of d2] (d3)
      {$p_\text{act}{\geq}0.40$\\$\wedge\;s{>}0.049$?};
  \node[decision, below=0.58cm of d3] (d4)
      {$s{>}0.052$?};

  \draw[arr] (in.south) -- ++(0,-0.15) -| (d1.north);
  \draw[arr] (d1) -- node[right,font=\tiny]{No} (d2);
  \draw[arr] (d2) -- node[right,font=\tiny]{No} (d3);
  \draw[arr] (d3) -- node[right,font=\tiny]{No} (d4);

  \node[alert,fill=red!30,   right=0.45cm of d1] (r1) {CRITICAL};
  \node[alert,fill=orange!40,right=0.45cm of d2] (r2) {HIGH};
  \node[alert,fill=yellow!50,right=0.45cm of d3] (r3) {MEDIUM};
  \node[alert,fill=green!30, right=0.45cm of d4] (r4) {LOW};
  \node[alert,fill=green!15, below=0.5cm of d4]  (r5) {LOW (default)};
  ...
  \draw[arr] (d4.south) -- node[right,font=\tiny]{No} (r5.north);

  % MLP path
  \node[mlpbox, fill=purple!15, below right=0.65cm and 0.6cm of in,
        minimum width=3cm, minimum height=1.1cm]
      (mlp) {MLP Head\\$3\to32\to16\to4$\\Softmax};

  \draw[arr] (in.south) -- ++(0,-0.15) -| (mlp.north);

  % Ensemble
  \node[proc, fill=teal!15, below=4.9cm of in, minimum width=5.5cm]
      (ens) {Ensemble: $\ell^*=\min(\ell_\text{rule},\;\ell_\text{MLP})$};

  \draw[arr] (r1.south) -- ++(0,-0.15) |- (ens.west);
  \draw[arr] (r2.south) -- ++(0,-0.10) |- (ens.west);
  \draw[arr] (r3.south) -- ++(0,-0.05) |- (ens.west);
  \draw[arr] (r4.south)                |- (ens.west);
  \draw[arr] (r5.south) -- ++(0,-0.15) |- (ens.west);
  \draw[arr] (mlp.south) -- ++(0,-3.2) -| (ens.east);

\end{tikzpicture}}
\caption{Fusion layer decision flow. The rule-based cascade (left) and MLP
head (right) run in parallel; the ensemble escalates to the higher-severity
output.}
\label{fig:fusion}
\end{figure}
...
We report on the \textbf{45-video subset} for which real VideoMAE predictions
are available (13 action classes + Normal).
...
Table~\ref{tab:mae} shows per-class VideoMAE performance on UCF-Crime.
Robbery (F1=0.53) and Road Accidents (F1=0.51) achieve the highest scores;
visually-subtle classes such as Shooting (F1=0.10) and Arrest (F1=0.11)
remain challenging due to class imbalance (13--35 support each).

\begin{table}[h]
\caption{VideoMAE Per-Class Performance on UCF-Crime (selected classes)}
\label{tab:mae}
\centering
\begin{tabular}{lcccc}
\toprule
...
\midrule
Robbery        & 0.44 & 0.66 & \textbf{0.53} & 35\\
...
precision $>0.99$ and FPR $<0.02$ simultaneously.
Gait-only achieves perfect recall but 100\% FPR, showing it cannot
discriminate without cross-modal support; VideoMAE-only achieves zero FPR
but misses 92.9\% of anomalies.
...
VideoMAE-Only         & 1.000 & 0.071 & 0.133 & 0.000 &  8.0\\
...
\textbf{Full (MLP)}   & \textbf{0.998} & \textbf{0.895}
    & \textbf{0.943} & \textbf{0.018} & 19.5\\
\bottomrule
\end{tabular}
\end{table}
...
baseline.
The MLP fusion achieves a 95.8\% FP reduction, lowering the false-alarm rate
from 43.9\% (YOLO without persistence, baseline) to 1.8\%.
...
\textbf{Full System (MLP)} & \textbf{1.8\%} & \textbf{95.8\%}\\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Gait-YOLO (ours)} & \textbf{Obj+Act+Gait} & \textbf{F1=0.943} & \textbf{95.8\%}\\
\bottomrule
...
VideoMAE-only reaches FPR=0 but recalls only 7.1\% of anomalies;
Gait-only recalls everything but with 100\% FPR.
...
\textbf{Limitations.}
The VideoMAE module achieves 0.40 overall accuracy on UCF-Crime, limited by
class imbalance for rare categories (Shooting, Arrest).
...
Evaluated on three real-world datasets, the system achieves fusion F1=0.943
with precision 0.998 and reduces the false-positive rate from 43.9\% to
1.8\% (95.8\% reduction) versus YOLO-only operation, while running at
