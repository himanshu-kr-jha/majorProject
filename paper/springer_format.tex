...
$\text{Linear}(3{\to}32){\to}\text{ReLU}{\to}\text{Dropout}(0.3){\to}\text{Linear}(32{\to}16){\to}\text{ReLU}{\to}\text{Linear}(16{\to}4){\to}\text{Softmax}$
on the normalised score vector $[c_\text{yolo},\,p_\text{act},\,s/0.06]$. Training uses 10{,}000 bootstrap samples labelled by the cascade (50 epochs, Adam, lr = 1e-3, cross-entropy). The MLP picks up cases the rules miss because of hard threshold edges. We combine the two by taking the higher-severity prediction:
\begin{equation}
  \ell^* = \min(\ell_\text{rule},\,\ell_\text{MLP})
\end{equation}
with level index 0 = Critical, 3 = Low. The cascade acts as a safety net (severity never drops); the MLP acts as a lift.
...
\textbf{Metrics.} Precision, recall, F1, mAP@0.50 (YOLO), false-positive rate. At fusion level any alert at HIGH or above is a positive.

\section{Results}\label{sec:results}

\subsection{Per-Module Performance}

\textbf{YOLO} (Table~\ref{tab:yolo}). mAP@0.50 = 0.724 on the 324 test images; lower than the 0.819 on the validation set as expected, due to the fact that the test set is partially out-of-distribution. Knife AP (0.739) is better than a pistol (0.709), consistent with the literature: at CCTV resolution a knife silhouette is more distinctive than a hand-held pistol.
...
\textbf{Gait} (Table~\ref{tab:gait}). With $\tau^* = 0.0642$, the branch reaches P = 0.746, R = 0.843, F1 = 0.792. The two distributions have a $\Delta\mu / \sigma_\text{nm} \approx 1.7$ separation, larger than the simulation-derived estimate, which is what lets the calibrated threshold reach F1 = 0.792 against the F1 = 0.665 of the simulation-derived threshold.
...
\begin{axis}[
  ...
  legend style={font=\scriptsize, at={(0.5,1.0)}, anchor=north, legend columns=2},
...
The YOLO baseline has FPR = 43.9\% on weapon images; on UCF-Crime it drops to 0\% because weapons rarely occur in the normal subset, which makes them a clear trigger for the CRITICAL channel. VideoMAE alone has FPR = 16\% on UCF-Crime, and the eight false positives are all motion-heavy scenes (crowds, traffic, sport). Table~\ref{tab:compare} places Gait-YOLO next to representative prior systems; the MLP fusion variant gives a substantial FP reduction at comparable detection accuracy.
...
{\scriptsize\textbf{(a) True Positive --- Arson.} VideoMAE = 0.9998 $\gg \tau^*$ = 0.9654; gait = 0.067; Rule = HIGH, MLP = HIGH.}\\[3pt]
\includegraphics[width=0.78\textwidth]%
...
{\scriptsize\textbf{(c) False Positive --- Normal corridor.} VideoMAE = 0.987 $> \tau^*$ from ambient motion; YOLO = 0.148 and gait = 0.066 do not corroborate.}
\caption{Qualitative fusion decisions on UCF-Crime test clips (4 sampled frames per strip). (a) all three branches agree, (b) cross-modal corroboration recovers a VideoMAE-alone false negative, (c) over-confident action signal without modality support remains the dominant failure mode.}
\label{fig:qualitative}
\end{figure}

\subsection{Operating Curves and Distributions}

Fig.~\ref{fig:pr} shows precision--recall for YOLOv8n on the 324 weapon test images; knife dominates pistol at every recall point, consistent with Table~\ref{tab:yolo}. Fig.~\ref{fig:gait_dist} plots the gait reconstruction-error distributions: the means are close, but the abnormal class has $\approx 2.2\times$ the variance of the normal class, which is what the data-calibrated $\tau^*$ exploits.
...
\addplot[mark=*, mark size=1.8pt, only marks, black]
  coordinates {(0.757, 0.799)};
\end{axis}
\end{tikzpicture}
\caption{PR curves for YOLOv8n on 324 weapon test images. The operating point (P = 0.799, R = 0.757) is marked.}
\label{fig:pr}
\end{minipage}\hfill
\begin{minipage}[t]{0.49\textwidth}
\centering
...
\begin{axis}[
  ...
  grid=major, grid style={dashed,gray!30},
  legend style={font=\tiny, at={(0.5,1.0)}, anchor=north, legend columns=1},
  tick label style={font=\tiny}, label style={font=\tiny},
  scaled x ticks=false,
  xticklabel style={/pgf/number format/fixed,/pgf/number format/precision=3, rotate=30, anchor=east},
  xtick={0.055,0.065,0.075,0.085},
]
\addplot[blue, thick, domain=0.050:0.092, samples=100]
  { exp(-0.5*((x-0.0632)/0.0039)^2) / (0.0039*2.5066) * 0.003 };
\addlegendentry{Normal}
\addplot[red!70, thick, dashed, domain=0.050:0.092, samples=100]
  { exp(-0.5*((x-0.0699)/0.0059)^2) / (0.0059*2.5066) * 0.003 };
\addlegendentry{Abnormal}
\addplot[black, dotted, very thick] coordinates {(0.0642,0)(0.0642,0.35)};
\end{axis}
\end{tikzpicture}
\caption{Gait reconstruction-error distributions on CASIA-B ($n = 600$). Abnormal sequences are shifted right ($\Delta\mu = 0.0067$); $\tau^* = 0.0642$ achieves F1 = 0.792.}
\label{fig:gait_dist}
\end{minipage}
\end{figure}

\textbf{Computational performance.} The asynchronous schedule reaches 18--22\,FPS on a single NVIDIA T4. CRITICAL alerts surface in $\approx 0.2$\,s (five-frame persistence) and the behavioural alerts in $\approx 0.8$\,s (16-frame VideoMAE clip). Per-frame end-to-end latency stays under 60\,ms because the slower transformers run on the every-fourth-frame schedule.
